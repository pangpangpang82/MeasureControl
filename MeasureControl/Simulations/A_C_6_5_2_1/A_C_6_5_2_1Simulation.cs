...
namespace MeasureControl.Simulations.A_C_6_5_2_1
{
    public sealed class A_C_6_5_2_1Simulation : ARINC429SimulationBase
    {
        ...
        private static readonly byte[] SArinc429In01Command8 = { 0x13, 0x02, 0x01, 0x01, 0x00, 0x00, 0x00, 0x00 };
        private static readonly byte[] SArinc429In01OkPrefix4 = { 0x13, 0x02, 0x01, 0x02 };

        private static readonly byte[] AbArx0ReceiveCommand8 = { 0x04, 0x02, 0x01, 0x01, 0x00, 0x00, 0x00, 0x00 };
        private static readonly byte[] AbArx0ReceiveOkPrefix4 = { 0x04, 0x02, 0x01, 0x02 };
        ...
        private async Task ProcessSimRxAsync(Action<string> log, CancellationToken token)
        {
            ...
            foreach (var item in list)
            {
                ...
                // 32位ARINC429数据：当不是命令分片(label=0x31..0x34)也不是旧的2帧拼包/回包(label=0x09..0x0C)时，直接缓存完整32位数据
                ...
                if (_cmdAssembler.TryAddFragment(label, payload, now, out var cmd8) && cmd8 != null)
                {
                    ...
                    if (cmd8.SequenceEqual(SArinc429In01Command8))
                    {
                        var data = _cachedReceivedData4 ?? TestData4.ToArray();
                        var resp8 = new byte[8];
                        Buffer.BlockCopy(SArinc429In01OkPrefix4, 0, resp8, 0, 4);
                        Buffer.BlockCopy(data, 0, resp8, 4, 4);
                        resp8[7] = 0x80;

                        log?.Invoke($"[{DateTime.Now:HH:mm:ss}] [SIM] 产品侧收到S_ARINC429_IN01 -> 回传缓存数据 resp8={FormatBytes(resp8)}");
                        await SendMultiLabelFrameOnChannelAsync(SimProductTxChannelIndex, ProductTxFragmentLabels, SwapPairs8(resp8), log, token);
                        continue;
                    }

                    if (cmd8.SequenceEqual(AbArx0ReceiveCommand8))
                    {
                        var data = _cachedReceivedData4 ?? TestData4.ToArray();
                        var resp8 = new byte[8];
                        Buffer.BlockCopy(AbArx0ReceiveOkPrefix4, 0, resp8, 0, 4);
                        Buffer.BlockCopy(data, 0, resp8, 4, 4);
