...
namespace MeasureControl.Simulations.A_C_6_5_1_1
{
    public sealed class A_C_6_5_1_1Simulation : ARINC429SimulationBase
    {
        ...
        public async Task<byte[]> WaitBenchResponse4Async(string benchRxChannel, int timeoutMs, Action<string> log, CancellationToken token)
        {
            ...
            return await WaitBenchData4Async(
                benchRxChannel,
                ProductTxFragmentLabels[0],
                ProductTxFragmentLabels[1],
                ProductTxFragmentLabels[2],
                ProductTxFragmentLabels[3],
                timeoutMs,
                log,
                token);
        }
            ...
            foreach (var item in list)
            {
                ...
                if (_cmdAssembler.TryAddFragment(label, payload, DateTime.UtcNow, out var cmd8) && cmd8 != null)
                {
                    ...
                    if (cmd8.SequenceEqual(SArinc429OutCommand8))
                    {
                        ...
                        await SendSingleWordOnChannelAsync(SimProductTxChannelIndex, 0x7F00AAAA, log, token);
                        continue;
                    }
                }
            }
