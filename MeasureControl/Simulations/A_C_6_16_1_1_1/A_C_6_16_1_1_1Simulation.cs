...
namespace MeasureControl.Simulations.A_C_6_16_1_1_1
{
    public sealed class A_C_6_16_1_1_1Simulation : ARINC429SimulationBase
    {
                ...
                try
                {
                    ...
                    if (list != null && list.Count > 0)
                    {
                        foreach (var item in list)
                        {
                            ...
                            if (cmd8.SequenceEqual(Speed1Command8))
                            {
                                log?.Invoke($"[{DateTime.Now:HH:mm:ss}] [SIM] 产品侧收到Speed1 -> 发送回包");
