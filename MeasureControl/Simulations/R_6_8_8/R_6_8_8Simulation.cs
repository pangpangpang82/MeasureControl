...
namespace MeasureControl.Simulations.R_6_8_8
{
    public sealed class R_6_8_8Simulation : IDisposable
    {
        ...
        private static readonly byte[] TemperatureTestCommand = { 0x07, 0x01, 0x08, 0x01, 0x00, 0x00, 0x00, 0x00 };
        ...
        public async Task StartAsync(string benchTxChannel, string benchRxChannel, Action<string> log)
        {
            ...
            log($"[{DateTime.Now:HH:mm:ss}] [SIM] PTS 仿真初始化开始");
            ...
            _started = true;
            log($"[{DateTime.Now:HH:mm:ss}] [SIM] PTS 仿真初始化完成");
        }
        ...
        public async Task StopAsync(Action<string> log)
        {
            ...
            log($"[{DateTime.Now:HH:mm:ss}] [SIM] PTS 仿真停止：释放设备资源");
        ...
        {
            // Assume standard ambient temperature (10-50°C) for simulation
            // Using the stricter ranges for standard ambient conditions
            return gear switch
            {
                "1挡" => (-77.05, -72.95),
                "2挡" => (23.63, 27.73),
