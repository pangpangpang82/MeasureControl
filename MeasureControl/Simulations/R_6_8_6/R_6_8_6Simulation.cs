...
namespace MeasureControl.Simulations.R_6_8_6
{
    public sealed class R_6_8_6Simulation : IDisposable
    {
        ...
        private static readonly byte[] TemperatureTestCommand = { 0x07, 0x01, 0x06, 0x01, 0x00, 0x00, 0x00, 0x00 };
        ...
        public async Task StartAsync(string benchTxChannel, string benchRxChannel, Action<string> log)
        {
            ...
            log($"[{DateTime.Now:HH:mm:ss}] [SIM] CAR_DTS 仿真初始化开始");
            ...
            _started = true;
            log($"[{DateTime.Now:HH:mm:ss}] [SIM] CAR_DTS 仿真初始化完成");
        }
        ...
        public async Task StopAsync(Action<string> log)
        {
            ...
            log($"[{DateTime.Now:HH:mm:ss}] [SIM] CAR_DTS 仿真停止：释放设备资源");
                        ...
                        foreach (var item in list)
                        {
                            if (TryParseWord(item.Data429, out byte label, out byte sdi, out ushort payload))
                            {
                                if (_rxLabelAssembler.TryAddFragment(label, payload, DateTime.UtcNow, out var cmd8) && cmd8 != null)
                                {
                                    ...
                                    {
                                        log($"[{DateTime.Now:HH:mm:ss}] [SIM] 产品侧收到 AB_CAR_DTS_Temperature 指令 -> 回复确认并开启遥测");
