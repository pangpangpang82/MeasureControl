...
namespace MeasureControl.Simulations.S_C_8_7_3
{
    public sealed class S_C_8_7_3Simulation : ARINC429SimulationBase
    {
                    ...
                    if (list != null && list.Count > 0)
                    {
                        foreach (var item in list)
                        {
                            ...
                            if (_rxLabelAssembler.TryAddFragment(label, payload, DateTime.UtcNow, out var cmd8) && cmd8 != null)
                            {
                                ...
                                else if (cmd8.SequenceEqual(SAftAventsMea038))
                                {
                                    log?.Invoke($"[{DateTime.Now:HH:mm:ss}] [SIM] 产品侧收到S_AFTAVENTS_MEA03 -> 启动周期遥测发送");
                                    StartTelemetryOutput();
                                    await SendMultiLabelFrameOnChannelAsync(SimProductTxChannelIndex, ProductTxFragmentLabels, SwapPairs8(SAftAventsMea038), log, token);
                                }
                            }
                        }
                    }
