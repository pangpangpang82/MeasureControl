                    ...
                    if (list != null && list.Count > 0)
                    {
                        foreach (var item in list)
                        {
                            ...
                            if (_rxLabelAssembler.TryAddFragment(label, payload, DateTime.UtcNow, out var cmd8) && cmd8 != null)
                            {
                                ...
                                {
                                    log?.Invoke($"[{DateTime.Now:HH:mm:ss}] [SIM] 产品侧收到S_AFTAVENTS_MEA -> 启动周期遥测发送");
                                    StartTelemetryOutput();
                                    await SendMultiLabelFrameOnChannelAsync(SimProductTxChannelIndex, ProductTxFragmentLabels, SAftAventsMea038, log, token);
                                }
                            }
                        }
                    }
