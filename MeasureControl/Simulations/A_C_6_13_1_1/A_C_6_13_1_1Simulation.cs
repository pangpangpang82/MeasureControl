...
namespace MeasureControl.Simulations.A_C_6_13_1_1
{
    public sealed class A_C_6_13_1_1Simulation : ARINC429SimulationBase
    {
        ...
        private static readonly byte[] PressureTestCommand8 = { 0x07, 0x06, 0x01, 0x01, 0x00, 0x00, 0x00, 0x00 };
            ...
            while (!token.IsCancellationRequested && DateTime.UtcNow <= deadline)
            {
                ...
                if (list != null && list.Count > 0)
                {
                    foreach (var item in list)
                    {
                        ...
                        if (labelAssembler.TryAddFragment(rxLabel, payload, DateTime.UtcNow, out var resp8) && resp8 != null)
                        {
                            if (resp8.Length == 8 && IsPrefix(resp8, PressureResponsePrefix4))
                            {
                                log?.Invoke($"[{DateTime.Now:HH:mm:ss}] [SIM] 压力回包拼包完成：{FormatBytes(resp8)}");
                                return resp8;
                            }
                        }
                    }
                    ...
                    if (list != null && list.Count > 0)
                    {
                        foreach (var item in list)
                        {
                            ...
                            if (_rxLabelAssembler.TryAddFragment(label, payload, DateTime.UtcNow, out var cmd8) && cmd8 != null)
                            {
                                ...
                                {
                                    log?.Invoke($"[{DateTime.Now:HH:mm:ss}] [SIM] 产品侧收到压力测试命令 -> 回复一次压力值");
                                    var resp8 = BuildPressureResponsePayload8();
        ...
        private byte[] BuildPressureResponsePayload8()
        {
            int gearIndex = 1;
            ...
            payload[0] = PressureResponsePrefix4[0];
            payload[1] = PressureResponsePrefix4[1];
            payload[2] = PressureResponsePrefix4[2];
            payload[3] = PressureResponsePrefix4[3];
