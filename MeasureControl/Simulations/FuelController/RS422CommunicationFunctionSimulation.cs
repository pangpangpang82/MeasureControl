...
{
    public sealed class RS422CommunicationFunctionSimulation : IDisposable
    {
        ...
        public async Task<bool> ConnectMatrixAsync(Action<string> log, CancellationToken token = default)
        {
            ...
            try
            {
                ...
                log?.Invoke("[SIM] 正在配置矩阵开关通路(RS422占位)...");
                bool ok = await MatrixControlService.Instance.ConnectNodesAsync("I3", "O30", MatrixSlotRs422, MatrixIpAddress);
                log?.Invoke($"[SIM] 矩阵开关通路(RS422): I3->O30 slot={MatrixSlotRs422} ip={MatrixIpAddress}, ok={ok}");
        ...
        public async Task DisconnectMatrixAsync(Action<string> log, CancellationToken token = default)
        {
            ...
            try
            {
                ...
                log?.Invoke($"[SIM] 矩阵开关断开(RS422): I3->O30 slot={MatrixSlotRs422}, ok={ok}");
        ...
        public async Task<byte[]> SendAndReceiveAsync(string stepName, byte[] txData, Action<string> log, CancellationToken token = default)
        {
            if (txData == null)
            ...
            log?.Invoke($"[SIM] {stepName}: 发送 0x{hex}");

            await Task.Delay(120, token);

            // 占位仿真：直接回环回传
            var rx = txData.ToArray();
            log?.Invoke($"[SIM] {stepName}: 回读 0x{hex}");
            return rx;
        }
