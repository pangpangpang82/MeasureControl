...
namespace MeasureControl.Simulations.A_C_6_10_6_1
{
    public sealed class A_C_6_10_6_1Simulation : ARINC429SimulationBase
    {
        ...
        private static readonly byte[] PressureTestCommand8 = { 0x07, 0x03, 0x06, 0x01, 0x00, 0x00, 0x00, 0x00 };
            ...
            while (!token.IsCancellationRequested && DateTime.UtcNow <= deadline)
            {
                ...
                if (list != null && list.Count > 0)
                {
                    foreach (var item in list)
                    {
                        ...
                        if (labelAssembler.TryAddFragment(rxLabel, payload, DateTime.UtcNow, out var resp8) && resp8 != null)
                        {
                            if (resp8.Length == 8 && IsPrefix(resp8, PressureTelemetryPrefix4))
                            {
                                log?.Invoke($"[{DateTime.Now:HH:mm:ss}] [SIM] 压力遥测拼包完成：{FormatBytes(resp8)}");
                                return resp8;
                            }
                        }
                    }
