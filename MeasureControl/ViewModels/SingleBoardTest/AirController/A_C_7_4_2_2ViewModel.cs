...
{
    public sealed class A_C_7_4_2_2ViewModel : BindableBase, IDisposable
    {
        ...
        private const string Slot2601_2_J33_Out = "O14";
        private const string Slot2601_2_J35_Out = "O15";

        // 2601(1) -> scope routing
        ...
        // 临时复用 7.4.1.2 的 FPGA 指令（后续可再调整）
        private static readonly byte[] DeviceInitCommandFrame = { 0xAA, 0x55, 0x02, 0x02, 0x01 };
        private static readonly byte[] DirHighCommand = { 0xAA, 0x55, 0x06, 0x04, 0x06, 0xE8, 0x03, 0x00, 0x00 };
        private static readonly byte[] DirLowCommand = { 0xAA, 0x55, 0x06, 0x04, 0x02, 0xE8, 0x03, 0x00, 0x00 };
        private static readonly byte[] ResetToInitialCommandFrame = { 0xAA, 0x55, 0x06, 0x04, 0x00, 0xE8, 0x03, 0xE8, 0x03 };
        ...
        private bool _matrixRouted2601_2_J33;
        private bool _matrixRouted2601_2_J35;
        ...
        private async Task StartAutoTestAsync()
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    var overallOk = ok1 && pass2 && ok3 && pass4;
        ...
        private async Task MeasurePhaseHighAsync()
        {
            ...
            try
            {
                ...
                try
                {
                    var (ok, phase) = await MeasurePhaseDiffDegAsync(CancellationToken.None).ConfigureAwait(false);
                    PhaseHighText = phase.HasValue ? $"{phase.Value:F1} °" : "--";
                    var pass = ok && phase.HasValue && IsPhaseMatch(phase.Value, ExpectedPhaseHighDeg, PhaseToleranceDeg);
                    Step2Result = pass ? "PASS" : "FAIL";
                    AddLog($"DIR高电平相位差: {PhaseHighText}  判据: {ExpectedPhaseHighDeg:F0}°±{PhaseToleranceDeg:F0}° => {Step2Result}");
                    SetOverall(pass ? "PASS" : "FAIL");
                }
                finally
                {
                    IsBusy = false;
        ...
        private async Task SendDirLowAsync()
        {
            ...
            try
            {
                ...
                try
                {
                    var ok = await SendFpgaCommandAsync(DirLowCommand, "DIR低电平", CancellationToken.None).ConfigureAwait(false);
                    AddLog($"DIR低电平指令: {(ok ? "PASS" : "FAIL")}");
                }
                finally
                {
                    IsBusy = false;
        ...
        private async Task MeasurePhaseLowAsync()
        {
            ...
            try
            {
                ...
                {
                    var (ok, phase) = await MeasurePhaseDiffDegAsync(CancellationToken.None).ConfigureAwait(false);
                    PhaseLowText = phase.HasValue ? $"{phase.Value:F1} °" : "--";
                    var pass = ok && phase.HasValue && IsPhaseMatch(phase.Value, ExpectedPhaseLowDeg, PhaseToleranceDeg);
        ...
        private async Task<(bool ok, double? phaseDeg)> MeasurePhaseDiffDegAsync(CancellationToken token)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    AddLog("路由测相位：J33->CH1, J35->CH2");

                    okJ33 = await matrix.ConnectNodesAsync(Slot2601_2_In, Slot2601_2_J33_Out, Chassis2Slot2601_2, DefaultMatrixIpAddress, DefaultMatrixTcpBasePort).ConfigureAwait(false);
                    okJ35 = await matrix.ConnectNodesAsync(Slot2601_2_In, Slot2601_2_J35_Out, Chassis2Slot2601_2, DefaultMatrixIpAddress, DefaultMatrixTcpBasePort).ConfigureAwait(false);
                    ...
                    _matrixRouted2601_2_J33 = okJ33;
                    _matrixRouted2601_2_J35 = okJ35;
                    _matrixRouted2601_1_Ch1 = okCh1;
                    _matrixRouted2601_1_Ch2 = okCh2;
                    _isMatrixRouted = okJ33 && okJ35 && okCh1 && okCh2;

                    AddLog($"路由结果：2601(2)J33={(okJ33 ? "OK" : "FAIL")}, J35={(okJ35 ? "OK" : "FAIL")}; 2601(1)CH1={(okCh1 ? "OK" : "FAIL")}, CH2={(okCh2 ? "OK" : "FAIL")}");
                    if (!_isMatrixRouted)
                        return (false, null);
        ...
        {
            if (!_isMatrixRouted && !_matrixRouted2601_2_J33 && !_matrixRouted2601_2_J35 && !_matrixRouted2601_1_Ch1 && !_matrixRouted2601_1_Ch2)
                return;
            ...
            try
            {
                if (_matrixRouted2601_2_J35)
                    _ = await matrix.DisconnectNodesAsync(Slot2601_2_In, Slot2601_2_J35_Out, Chassis2Slot2601_2, DefaultMatrixIpAddress, DefaultMatrixTcpBasePort).ConfigureAwait(false);
            }
            catch { }

            try
            {
                if (_matrixRouted2601_2_J33)
                    _ = await matrix.DisconnectNodesAsync(Slot2601_2_In, Slot2601_2_J33_Out, Chassis2Slot2601_2, DefaultMatrixIpAddress, DefaultMatrixTcpBasePort).ConfigureAwait(false);
            }
            catch { }

            _isMatrixRouted = false;
            _matrixRouted2601_2_J33 = false;
            _matrixRouted2601_2_J35 = false;
