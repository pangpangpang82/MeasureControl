    ...
    public class S_C_8_6_2ViewModel : BindableBase, IDisposable
    {
        ...
        private async Task OnTestTemperatureTelemetryAsync()
        {
            ...
            try
            {
                ...
                if (temperature == null)
                {
                    AddLog($"[{DateTime.Now:HH:mm:ss}] 温度回采值失败：缓存中无温度采集值(07 01 03 02)");
                ...
                if (rawData != null)
                {
                    ...
                    if (TryParseBase6FromNibbles(rawData, out var rawBase6Decimal))
                        AddLog($"[{DateTime.Now:HH:mm:ss}] WAITS2温度原始数据(07 01 03 03) 后四字节(6进制)->10进制：{rawBase6Decimal}，Data={rawHex}");
                    else
                        AddLog($"[{DateTime.Now:HH:mm:ss}] WAITS2温度原始数据(07 01 03 03) Data={rawHex}");
                }
            }
            catch (Exception ex)
            {
