...
using MeasureControl.Simulations.S_C_8_6_2;

namespace MeasureControl.ViewModels.SingleBoardTest.AirController
{
    public class S_C_8_6_2ViewModel : BindableBase, IDisposable
    {
        ...
        private static readonly byte[] TestCommand = { 0x15, 0x01, 0x02, 0x01, 0x00, 0x00, 0x00, 0x00 };
        ...
        private static readonly byte[] TelemetryTemperaturePrefix = { 0x15, 0x01, 0x02, 0x02 };
        private static readonly byte[] TelemetryRawPrefix = { 0x15, 0x01, 0x02, 0x03 };

        public S_C_8_6_2ViewModel()
        {
            _enterAtpTxChannel = FixedTxChannel;
            _enterAtpRxChannel = FixedRxChannel;
            _controllerTemperatureTestTxChannel = FixedTxChannel;
            _temperatureTelemetryRxChannel = FixedRxChannel;
        ...
        private readonly S_C_8_6_2Simulation _simulation = new S_C_8_6_2Simulation();
        ...
        public string EnterAtpTxDataText
        {
            get => "0x" + FormatBytesHex(AtpR);
        }
        ...
        public string ControllerTemperatureTestTxDataText
        {
            get => "0x" + FormatBytesHex(TestCommand);
        }
        ...
        public string ExitAtpTxDataText
        {
            get => "0x" + FormatBytesHex(AtpE);
        }
        ...
        private async Task<bool> AutoEnterAtpAsync(CancellationToken token)
        {
            ...
            await _simulation.SendBenchCommandOnlyAsync(
                EnterAtpTxChannel, DefaultLabel, AtpR,
        ...
        private async Task<bool> AutoExitAtpAsync(CancellationToken token)
        {
            ...
            await _simulation.SendBenchCommandOnlyAsync(
                ExitAtpTxChannel, DefaultLabel, AtpE,
                msg => AddLog(msg), token);
        ...
        {
            if (!IsManualTestRunning || _isBusy)
                return;
            ...
            try
            {
                ...
                try
                {
                    ...
                    AddLog($"[{DateTime.Now:HH:mm:ss}] 发送进入ATP：{FormatBytes(AtpR)}");
                    await _simulation.SendBenchCommandOnlyAsync(EnterAtpTxChannel, DefaultLabel, AtpR, msg => AddLog(msg), token);
        ...
        private (double Min, double Max) GetQualifiedTemperatureRangeForGear(string gear)
        {
            ...
            return gear switch
            {
                "1挡" => ambient ? (-77.05, -72.95) : (-79.05, -70.95),
                "2挡" => ambient ? (23.63, 27.73) : (21.63, 29.73),
                "3挡" => ambient ? (97.95, 102.05) : (95.95, 104.05),
        ...
        private async Task OnSendExitAtpAsync()
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    AddLog($"[{DateTime.Now:HH:mm:ss}] 发送退出ATP：{FormatBytes(AtpE)}");
                    await _simulation.SendBenchCommandOnlyAsync(ExitAtpTxChannel, DefaultLabel, AtpE, msg => AddLog(msg), token);
        ...
        private async Task OnTestControllerTemperatureAsync()
        {
            ...
            try
            {
                AddLog($"[{DateTime.Now:HH:mm:ss}] 测试：8.6.2测试指令，TX={ControllerTemperatureTestTxChannel}, Label=0x{DefaultLabel:X2}");
        ...
        private async Task OnTestTemperatureTelemetryAsync()
        {
            ...
            try
            {
                ...
                {
                    if (LastTelemetryTemperatureC != null && _lastTemperatureTelemetryFrame != null)
                        break;
                    await Task.Delay(50);
                }
                ...
                if (temperature == null)
                {
                    AddLog($"[{DateTime.Now:HH:mm:ss}] 温度回采值失败：缓存中无温度采集值(15 01 02 02)");
                ...
                if (rawData != null)
                {
                    ...
                    if (TryParseBase6FromNibbles(rawData, out var rawBase6Decimal))
                        AddLog($"[{DateTime.Now:HH:mm:ss}] WAITS2温度原始数据(15 01 02 03) 后四字节(6进制)->10进制：{rawBase6Decimal}，Data={rawHex}");
                    else
                        AddLog($"[{DateTime.Now:HH:mm:ss}] WAITS2温度原始数据(15 01 02 03) Data={rawHex}");
                }
            }
            catch (Exception ex)
            {
