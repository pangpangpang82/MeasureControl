    ...
    public class S_C_8_6_2ViewModel : BindableBase, IDisposable
    {
        ...
        private async Task OnTestControllerTemperatureAsync()
        {
            ...
            try
            {
                ...
                AddLog($"[{DateTime.Now:HH:mm:ss}] 测试：8.6.2测试指令，TX={ControllerTemperatureTestTxChannel}, RX={ControllerTemperatureTestRxChannel}, Label=0x{DefaultLabel:X2}");
                ...
                if (resp != null)
                {
                    ...
                    AddLog($"[{DateTime.Now:HH:mm:ss}] 8.6.2测试指令收到回包");
                }
            }
            catch (Exception ex)
            {
