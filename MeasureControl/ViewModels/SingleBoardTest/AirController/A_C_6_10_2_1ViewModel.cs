...

{
...

    {
        ...
        private static readonly string[] Mtx532EnabledAoChannels = { "AO2" };
        ...
        private async Task RunGearAutoAsync(int gearIndex, double voltageV, CancellationToken token, System.Collections.Generic.List<string> failures)
        {
            ...
            AddLog($"[{DateTime.Now:HH:mm:ss}] 自动测试：档位{gearIndex} AO2输出后等待稳定5s");
            ...
            AddLog($"[{DateTime.Now:HH:mm:ss}] 档位{gearIndex}：等待压力遥测(07 03 02 02)[持续监听]，timeout=7000ms");
            ...
            if (tel == null)
            {
                ...
                if (tel == null)
                {
                    failures.Add($"档位{gearIndex}压力遥测超时：RX={PressureTelemetryRxChannel}未收到label=0x90/0x50/0xD0/0x30且编码模板=07 03 02 02 00 00 00 00的回传帧");
                    return;
                }
            }
        ...
        private async Task OnTestGearAsync(int gearIndex)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    AddLog($"[{DateTime.Now:HH:mm:ss}] 等待压力遥测：RX={PressureTelemetryRxChannel}[持续监听]");
        ...
        private async Task OnSendControllerPressureTestAsync()
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    AddLog($"[{DateTime.Now:HH:mm:ss}] 测试指令已发送，等待压力遥测：RX={PressureTelemetryRxChannel}, 编码模板=07 03 02 02 00 00 00 00, timeout=8000ms");
                    ...
                    {
                        SetLastTestResult("FAIL");
                        AddLog($"[{DateTime.Now:HH:mm:ss}] 压力遥测超时：startSeq={startSeq}, currentSeq={currentSeq}, RX={PressureTelemetryRxChannel}, seq未增长，说明监听未收到产品回传压力帧。请检查产品是否在RX={PressureTelemetryRxChannel}回发label=0x90/0x50/0xD0/0x30且编码模板=07 03 02 02 00 00 00 00，并确认429接线/通道配置");
                        return;
                    }
...

        {
...

            {
                ...
                AddLog($"[{DateTime.Now:HH:mm:ss}] 档位{gearIndex}：直接等待压力遥测失败：{elapsed}ms内未收到07 03 02 02帧");
