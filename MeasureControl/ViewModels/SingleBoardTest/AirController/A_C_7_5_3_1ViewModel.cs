...
{
    public sealed class A_C_7_5_3_1ViewModel : BindableBase, IDisposable
    {
        ...
        private static readonly (string In, string Out, int Slot) MatrixPointSig = ("I1", "O22", MatrixSlotSig);
        ...
        private const int CurrentSet250mA = 250;

        private const double Voltage0mAMin = 0.0;
        private const double Voltage0mAMax = 0.41;

        private const double Voltage250mAMin = 9.36;
        private const double Voltage250mAMax = 10.66;

        private const double Current0mAMinA = 0.0;
        private const double Current0mAMaxA = 0.023;

        private const double Current250mAMinA = 0.227;
        private const double Current250mAMaxA = 0.273;
        ...
        public A_C_7_5_3_1ViewModel()
        {
            ...
            Step6Set250mACommand = new DelegateCommand(async () => await SendSetCurrentAndUpdateAsync(CurrentSet250mA, isFirst: false));
            Step7Measure250mACommand = new DelegateCommand(async () => await MeasureVoltageAndUpdateAsync(isFirst: false));
            Step8Readback250mACommand = new DelegateCommand(async () => await QueryCurrentReadbackAndUpdateAsync(isFirst: false));
        }
        ...
        public DelegateCommand Step6Set250mACommand { get; }
        public DelegateCommand Step7Measure250mACommand { get; }
        public DelegateCommand Step8Readback250mACommand { get; }
        ...
        private void RaiseAllCanExecuteChanged()
        {
            Application.Current?.Dispatcher?.Invoke(() =>
            {
                ...
                Step6Set250mACommand?.RaiseCanExecuteChanged();
                Step7Measure250mACommand?.RaiseCanExecuteChanged();
                Step8Readback250mACommand?.RaiseCanExecuteChanged();
            });
        }
        ...
        private async Task StartAutoTestAsync()
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    var ok6 = await SendSetCurrentAsync(CurrentSet250mA, token).ConfigureAwait(false);
                    AddLog($"250mA设定指令: {(ok6 ? "PASS" : "FAIL")}");
                    ...
                    AddLog($"250mA电压判定: {(pass7 ? "PASS" : "FAIL")}");
                    ...
                    AddLog($"250mA回采电流判定: {(pass8 ? "PASS" : "FAIL")}");
            ...
            try
            {
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        AddLog($"{(isFirst ? "0mA" : "250mA")}电压判定: {(pass ? "PASS" : "FAIL")}");
                    }
                    finally
                    {
                        IsBusy = false;
        ...
        private async Task QueryCurrentReadbackAndUpdateAsync(bool isFirst)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    AddLog($"{(isFirst ? "0mA" : "250mA")}回采电流判定: {(pass ? "PASS" : "FAIL")}");
                }
                finally
                {
                    IsBusy = false;
        ...
        private bool UpdateVoltageStepAndGetPass(bool isFirst, bool ok, double? voltage)
        {
            ...
            return v >= Voltage250mAMin && v <= Voltage250mAMax;
        }
        ...
        private bool UpdateCurrentStepAndGetPass(bool isFirst, bool ok, double? currentA, string replyHex)
        {
            ...
            return a >= Current250mAMinA && a <= Current250mAMaxA;
        }
        ...
        private static double? TryParseCurrentAFromReply(byte[] reply)
        {
            ...
            int hi = reply[reply.Length - 2];
            int lo = reply[reply.Length - 1];
            int mA = (hi << 8) | lo;
