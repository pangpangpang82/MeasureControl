...

{
...

    {
        ...
        private static readonly byte[] PressureTelemetryPrefix4 = { 0x07, 0x02, 0x02, 0x02 };
...

        {
...

            {
...

                {
                    ...
                    AddLog($"[{DateTime.Now:HH:mm:ss}] 发送AB_CABVENTS_Temperature：TX={ControllerPressureTestTxChannel}, RX={ControllerPressureTestRxChannel}, Data={FormatData(AbCabVentsTemperature8)}");
                    ...
                    var confirm = await _simulation.WaitBenchResponse8Async(
                        ...
                        b => b != null && b.SequenceEqual(AbCabVentsTemperature8),
...

        {
...

            {
...

                {
                    ...
                    AddLog($"[{DateTime.Now:HH:mm:ss}] 等待温度遥测(07 02 02 02)：RX={PressureTelemetryRxChannel}");
...

            {
                ...
                AddLog($"[{DateTime.Now:HH:mm:ss}] 温度回采异常：{ex.Message}");
...

        {
...

            {
...

                {
                    ...
                    await RunGearAutoAsync(1, 2.08, token, failures);

                    token.ThrowIfCancellationRequested();

                    await RunGearAutoAsync(2, 3.0, token, failures);

                    token.ThrowIfCancellationRequested();

                    await RunGearAutoAsync(3, 4.08, token, failures);
...

        {
            ...
            AddLog($"[{DateTime.Now:HH:mm:ss}] 档位{gearIndex}：等待温度遥测(07 02 02 02)");
...

        {
...

            {
...

                {
                    ...
                    AddLog($"[{DateTime.Now:HH:mm:ss}] 等待温度遥测：RX={PressureTelemetryRxChannel}");
