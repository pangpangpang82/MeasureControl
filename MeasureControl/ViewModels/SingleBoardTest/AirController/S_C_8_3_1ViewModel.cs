...
namespace MeasureControl.ViewModels.SingleBoardTest.AirController
{
    public sealed class S_C_8_3_1ViewModel : BindableBase, IDisposable
    {
        ...
        private const byte Label50 = 0x90;
        private const byte Label51 = 0x50;
        private const byte Label52 = 0xD0;
        private const byte Label53 = 0x30;
        ...
        private async Task OnSendSArinc429OutAsync()
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    AddLog($"[{DateTime.Now:HH:mm:ss}] 接收数据={RxDataText} 测试结果：{LastTestResult}");
                }
                finally
                {
                    IsBusy = false;
            ...
            try
            {
                ...
                try
                {
                    ...
                    if (resp8 == null || resp8.Length != 8)
                    {
                        SetLastTestResult("FAIL");
                        CurrentStepImage = CreateImageSource("/Resources/Logo/warning.png");
                        AddLog($"[{DateTime.Now:HH:mm:ss}] 自动测试失败：数据接收超时");
                        return;
                    }
        ...
        private static string FormatBytesHexCompact(byte[] bytes)
        {
            if (bytes == null || bytes.Length == 0)
                return string.Empty;
