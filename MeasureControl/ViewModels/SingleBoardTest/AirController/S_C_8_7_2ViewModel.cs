...
using MeasureControl.Simulations.S_C_8_7_2;

namespace MeasureControl.ViewModels.SingleBoardTest.AirController
{
    public sealed class S_C_8_7_2ViewModel : BindableBase, IDisposable
    {
        ...
        private static readonly byte[] TelemetryRawPrefix4 = { 0x15, 0x02, 0x02, 0x03 };
        ...
        private readonly S_C_8_7_2Simulation _simulation = new S_C_8_7_2Simulation();
        ...
        public string TestCommandTxDataText { get => "0x" + FormatData(SFwdAventsMea028); }
        ...
        private async Task OnTestGearAsync(int gearIndex)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    AddLog($"[{DateTime.Now:HH:mm:ss}] 档位{gearIndex}：AO4输出后等待稳定3s");
                    await Task.Delay(TimeSpan.FromSeconds(3));

                    AddLog($"[{DateTime.Now:HH:mm:ss}] 发送S_FWDAVENTS_MEA02：{FormatData(SFwdAventsMea028)}");
                    await _simulation.SendBenchCommandOnlyAsync(TestCommandTxChannel, SFwdAventsMea028, msg => AddLog(msg), CancellationToken.None);
        ...
        private async Task OnSendControllerPressureTestAsync()
        {
            ...
            try
            {
                ...
                try
                {
                    AddLog($"[{DateTime.Now:HH:mm:ss}] 发送S_FWDAVENTS_MEA02：TX={ControllerPressureTestTxChannel}");
                    await _simulation.SendBenchCommandOnlyAsync(ControllerPressureTestTxChannel, SFwdAventsMea028, msg => AddLog(msg), CancellationToken.None);
                }
                finally
                {
                    IsBusy = false;
        ...
        private async Task RunAutoGearAsync(int gearIndex, CancellationToken token, List<string> failures)
        {
            CurrentGearIndex = gearIndex;
            var voltageV = GetGearVoltage(gearIndex);
            AddLog($"[{DateTime.Now:HH:mm:ss}] 自动测试：{CurrentGearName} AO4={voltageV.ToString("0.###", CultureInfo.InvariantCulture)}V");
            ...
            AddLog($"[{DateTime.Now:HH:mm:ss}] 自动测试：{CurrentGearName} AO4输出后等待稳定1s");
        ...
        private async Task ResetMtx532OutputToZeroAsync()
        {
            ...
            try
            {
                ...
                await SetAo4Async(0.0, CancellationToken.None);
                VoltageSetValueText = "--";
            }
            catch (Exception ex)
            {
                AddLog($"[{DateTime.Now:HH:mm:ss}] MTX532复位AO4异常：{ex.Message}");
            }
            finally
            {
                _mtxOpLock.Release();
        ...
        private async Task<bool> EnsureMtx532ConnectedAsync(CancellationToken token = default)
        {
            ...
            AddLog($"[{DateTime.Now:HH:mm:ss}] MTX532连接流程开始：参考HC_6_5；硬件AO为1基，目标业务通道={AoChannel}（第4个物理通道）");
        ...
        private void LogTemperatureRawData(byte[] rawData)
        {
            ...
            if (TryParseBase6FromNibbles(rawData, out var rawBase6Decimal))
                AddLog($"[{DateTime.Now:HH:mm:ss}] FWD_AVENTS2温度原始数据(15 02 02 03) 后四字节(6进制)->10进制：{rawBase6Decimal}，Data={rawHex}");
            else
                AddLog($"[{DateTime.Now:HH:mm:ss}] FWD_AVENTS2温度原始数据(15 02 02 03) Data={rawHex}");
        }
