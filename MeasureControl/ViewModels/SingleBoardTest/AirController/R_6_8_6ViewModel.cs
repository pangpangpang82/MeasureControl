...
namespace MeasureControl.ViewModels.SingleBoardTest.AirController
{
    public class R_6_8_6ViewModel : BindableBase, IDisposable
    {
        ...
        private async Task OnTestControllerTemperatureAsync()
        {
            ...
            try
            {
                AddLog($"[{DateTime.Now:HH:mm:ss}] 测试：AB_CAR_DTS_Temperature，TX={ControllerTemperatureTestTxChannel}, Label=0x{DefaultLabel:X2}");
        ...
        private async Task OnTestTemperatureTelemetryAsync()
        {
            ...
            try
            {
                ...
                if (temperature == null)
                {
                    AddLog($"[{DateTime.Now:HH:mm:ss}] 温度回采值失败：缓存中无温度采集值(07 01 06 02)");
