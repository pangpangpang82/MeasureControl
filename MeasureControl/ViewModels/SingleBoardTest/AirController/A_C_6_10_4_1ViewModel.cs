...
using MeasureControl.Simulations.A_C_6_10_4_1;
...
namespace MeasureControl.ViewModels.SingleBoardTest.AirController
{
    ...
    public sealed class A_C_6_10_4_1ViewModel : BindableBase, IDisposable
    {
        ...
        private static readonly byte[] AbBpsPressure8 = { 0x07, 0x03, 0x04, 0x01, 0x00, 0x00, 0x00, 0x00 };

        private static readonly byte[] PressureTelemetryPrefix4 = { 0x07, 0x03, 0x04, 0x02 };
        ...
        private static readonly string[] Mtx532EnabledAoChannels = { "AO4" };
        ...
        private readonly A_C_6_10_4_1Simulation _simulation = new A_C_6_10_4_1Simulation();
        ...
        private async Task RunGearAutoAsync(int gearIndex, double voltageV, CancellationToken token, System.Collections.Generic.List<string> failures)
        {
            ...
            AddLog($"[{DateTime.Now:HH:mm:ss}] 自动测试：档位{gearIndex} AO4输出后等待稳定1s");
            ...
            AddLog($"[{DateTime.Now:HH:mm:ss}] 档位{gearIndex}：发送AB_WAIPS2_PRESSURE");
            await _simulation.SendBenchCommandOnlyAsync(TestTxChannel, AbBpsPressure8, msg => AddLog(msg), token);

            AddLog($"[{DateTime.Now:HH:mm:ss}] 档位{gearIndex}：等待压力遥测(07 03 04 02)[持续监听]，timeout=7000ms");
            ...
            if (tel == null)
            {
                ...
                AddLog($"[{DateTime.Now:HH:mm:ss}] 档位{gearIndex}：重发AB_WAIPS2_PRESSURE用于直接遥测读取");
                await _simulation.SendBenchCommandOnlyAsync(TestTxChannel, AbBpsPressure8, msg => AddLog(msg), token);
                ...
                if (tel == null)
                {
                    failures.Add($"档位{gearIndex}压力遥测超时：RX={PressureTelemetryRxChannel}未收到label=0x90/0x50/0xD0/0x30且编码模板=07 03 04 02 00 00 00 00的回传帧");
                    return;
                }
            }
        ...
        private async Task OnTestGearAsync(int gearIndex)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    AddLog($"[{DateTime.Now:HH:mm:ss}] 手动档位{gearIndex}：设置AO4={voltageV.ToString("0.###", CultureInfo.InvariantCulture)}V");
                    var okVoltage = await OutputVoltageAsync(voltageV, token);
                    if (!okVoltage)
                    {
                        SetLastTestResult("FAIL");
        ...
        private async Task<bool> OutputVoltageAsync(double voltageV, CancellationToken token)
        {
            ...
            try
            {
                ...
                AddLog($"[{DateTime.Now:HH:mm:ss}] MTX532档位电压写入开始：{AoChannel}={voltageV.ToString("0.####", CultureInfo.InvariantCulture)}V（1基，第4个物理通道）");
                await SetAo4Async(voltageV, token);
                ...
                var readBackVoltage = await ReadAo4VoltageAsync(token);
        ...
        private async Task<double?> ReadAo4VoltageAsync(CancellationToken token)
        {
        ...
        private async Task SetAo4Async(double voltageV, CancellationToken token)
        {
            if (_mtxApi == null || !_mtxApi.IsConnected)
        ...
        private async Task<bool> EnsureMtx532ConnectedAsync(CancellationToken token = default)
        {
            ...
            AddLog($"[{DateTime.Now:HH:mm:ss}] MTX532连接流程开始：硬件AO为1基，目标业务通道={AoChannel}（第4个物理通道）");
            ...
            AddLog($"[{DateTime.Now:HH:mm:ss}] MTX532写初始0V：{AoChannel}=0V（1基，第4个物理通道）");
            await SetAo4Async(0.0, token).ConfigureAwait(false);
        ...
        private async Task OnSendControllerPressureTestAsync()
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    AddLog($"[{DateTime.Now:HH:mm:ss}] 控制器压力单次测试：准备发送AB_WAIPS2_PRESSURE，TX={ControllerPressureTestTxChannel}, RX={PressureTelemetryRxChannel}, 当前档位={gearIndex}, IsInAtp={IsInAtp}");
                    ...
                    AddLog($"[{DateTime.Now:HH:mm:ss}] 发送AB_WAIPS2_PRESSURE：TX={ControllerPressureTestTxChannel}, Data={FormatData(AbBpsPressure8)}");
                    await _simulation.SendBenchCommandOnlyAsync(ControllerPressureTestTxChannel, AbBpsPressure8, msg => AddLog(msg), token);

                    AddLog($"[{DateTime.Now:HH:mm:ss}] 测试指令已发送，等待压力遥测：RX={PressureTelemetryRxChannel}, 编码模板=07 03 04 02 00 00 00 00, timeout=8000ms");
                    ...
                    {
                        SetLastTestResult("FAIL");
                        AddLog($"[{DateTime.Now:HH:mm:ss}] 压力遥测超时：startSeq={startSeq}, currentSeq={currentSeq}, RX={PressureTelemetryRxChannel}, seq未增长，说明监听未收到产品回传压力帧。请检查产品是否在RX={PressureTelemetryRxChannel}回发label=0x90/0x50/0xD0/0x30且编码模板=07 03 04 02 00 00 00 00，并确认429接线/通道配置");
                        return;
                    }
                    ...
                    var pass = IsPressureQualified(gearIndex, pressurePsia);
                    SetLastTestResult(pass ? "PASS" : "FAIL");
                    AddLog($"[{DateTime.Now:HH:mm:ss}] 控制器压力单次测试结束：档位{gearIndex}, 压力={pressurePsia.ToString("0.####", CultureInfo.InvariantCulture)}psia, 判定={LastTestResult}");
                }
                finally
                {
                    IsBusy = false;
        ...
        private async Task<byte[]> DirectWaitPressureTelemetryFrameAsync(int gearIndex, CancellationToken token)
        {
            ...
            if (tel == null)
            {
                ...
                AddLog($"[{DateTime.Now:HH:mm:ss}] 档位{gearIndex}：直接等待压力遥测失败：{elapsed}ms内未收到07 03 04 02帧");
                return null;
            }
