...
namespace MeasureControl.ViewModels.SingleBoardTest.AirController
{
    ...
    public sealed class A_C_6_10_4_1ViewModel : BindableBase, IDisposable
    {
        ...
        private async Task RunGearAutoAsync(int gearIndex, double voltageV, CancellationToken token, System.Collections.Generic.List<string> failures)
        {
            ...
            AddLog($"[{DateTime.Now:HH:mm:ss}] 档位{gearIndex}：等待压力遥测(07 03 04 02)");
        ...
        private async Task OnTestGearAsync(int gearIndex)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    AddLog($"[{DateTime.Now:HH:mm:ss}] 发送AB_WAIPS2_PRESSURE：TX={ControllerPressureTestTxChannel}");
        ...
        private async Task OnSendControllerPressureTestAsync()
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    AddLog($"[{DateTime.Now:HH:mm:ss}] 发送AB_WAIPS2_PRESSURE：TX={ControllerPressureTestTxChannel}, RX={ControllerPressureTestRxChannel}, Data={FormatData(AbBpsPressure8)}");
                    await _simulation.SendBenchCommandOnlyAsync(ControllerPressureTestTxChannel, AbBpsPressure8, msg => AddLog(msg), token);
        ...
        private async Task OnReadPressureTelemetryAsync()
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    AddLog($"[{DateTime.Now:HH:mm:ss}] 等待压力遥测(07 03 04 02)：RX={PressureTelemetryRxChannel}");
                    var tel = await _simulation.WaitPressureTelemetryAsync(PressureTelemetryRxChannel, timeoutMs: 1500, log: msg => AddLog(msg), token: token);
                    if (tel == null)
                    {
                        AddLog($"[{DateTime.Now:HH:mm:ss}] 压力遥测超时");
                    ...
                    PressureTelemetryValueText = pressurePsia.ToString("0.####", CultureInfo.InvariantCulture);
                    SetLastTestResult(IsPressureQualified(CurrentGearIndex, pressurePsia) ? "PASS" : "FAIL");
                }
                finally
                {
                    IsBusy = false;
