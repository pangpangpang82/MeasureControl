...
using MeasureControl.Simulations.R_6_8_4;
using Prism.Ioc;

namespace MeasureControl.ViewModels.SingleBoardTest.AirController
{
    public class R_6_8_4ViewModel : BindableBase, IDisposable
    {
        private const byte DefaultLabel = 0x6A;

        private const string ResistorChannelId = "RO2";
        ...
        private static readonly byte[] AbCkptDtsTemperature = { 0x07, 0x01, 0x04, 0x01, 0x00, 0x00, 0x00, 0x00 };
        private static readonly byte[] TelemetryTemperaturePrefix = { 0x07, 0x01, 0x04, 0x02 };
        private static readonly byte[] TelemetryRawPrefix = { 0x07, 0x01, 0x04, 0x03 };
        ...
        public R_6_8_4ViewModel()
        {
            ...
            AmbientTemperatureSelection = _ambientTemperatureSelection;
        ...
        private readonly R_6_8_4Simulation _simulation = new R_6_8_4Simulation();
        ...
        private async Task OnTestTemperatureTelemetryAsync()
        {
            ...
            try
            {
                ...
                if (temperature == null)
                {
                    AddLog($"[{DateTime.Now:HH:mm:ss}] 温度回采值失败：缓存中无温度采集值(07 01 04 02)");
                ...
                if (rawData != null)
                {
                    ...
                    if (TryParseBase6FromNibbles(rawData, out var rawBase6Decimal))
                        AddLog($"[{DateTime.Now:HH:mm:ss}] CKPT_DTS温度原始数据(07 01 04 03) 后四字节(6进制)->10进制：{rawBase6Decimal}，Data={rawHex}");
                    else
                        AddLog($"[{DateTime.Now:HH:mm:ss}] CKPT_DTS温度原始数据(07 01 04 03) Data={rawHex}");
                }
            }
            catch (Exception ex)
            {
