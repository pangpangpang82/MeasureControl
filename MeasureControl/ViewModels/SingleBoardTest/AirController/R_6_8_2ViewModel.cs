...
using MeasureControl.Simulations.R_6_8_2;
using NationalInstruments.Visa;
using Prism.Ioc;

namespace MeasureControl.ViewModels.SingleBoardTest.AirController
{
    public class R_6_8_2ViewModel : BindableBase, IDisposable
    {
        ...
        private static readonly byte[] AbMixtsTemperature = { 0x07, 0x01, 0x02, 0x01, 0x00, 0x00, 0x00, 0x00 };
        ...
        private readonly R_6_8_2Simulation _simulation = new R_6_8_2Simulation();
        ...
        private async Task OnTestControllerTemperatureAsync()
        {
            ...
            try
            {
                ...
                AddLog($"[{DateTime.Now:HH:mm:ss}] 测试：AB_MIXTS_Temperature，TX={ControllerTemperatureTestTxChannel}, RX={ControllerTemperatureTestRxChannel}, Label=0x{DefaultLabel:X2}");
                ...
                var resp = await _simulation.SendBenchCommandAndWaitAsync(
                    ...
                    DefaultLabel, AbMixtsTemperature,
                    b => b != null && b.Length == 8 && b.SequenceEqual(AbMixtsTemperature),
                    timeoutMs: 800,
                    msg => AddLog(msg), CancellationToken.None);
                ...
                if (resp != null)
                {
                    ...
                    AddLog($"[{DateTime.Now:HH:mm:ss}] AB_MIXTS_Temperature 收到回包");
                }
            }
            catch (Exception ex)
            {
        ...
        private async Task OnTestTemperatureTelemetryAsync()
        {
            ...
            try
            {
                ...
                if (tempData == null)
                {
                    AddLog($"[{DateTime.Now:HH:mm:ss}] 温度回采值失败：未收到温度采集值(07 01 02 02)");
                ...
                if (rawData != null)
                {
                    ...
                    if (TryParseBase6FromNibbles(rawData, out var rawBase6Decimal))
                        AddLog($"[{DateTime.Now:HH:mm:ss}] MIXTS温度原始数据(07 01 02 03) 后四字节(6进制)->10进制：{rawBase6Decimal}，Data={rawHex}");
                    else
                        AddLog($"[{DateTime.Now:HH:mm:ss}] MIXTS温度原始数据(07 01 02 03) Data={rawHex}");
                }
            }
            catch (Exception ex)
            {
