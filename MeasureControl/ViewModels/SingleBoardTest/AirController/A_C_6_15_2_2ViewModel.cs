...
using MeasureControl.Simulations.A_C_6_15_2_2;

namespace MeasureControl.ViewModels.SingleBoardTest.AirController
{
    public sealed class A_C_6_15_2_2ViewModel : BindableBase, IDisposable
    {
        ...
        private static readonly byte[] PhHighCommand8 = { 0x21, 0x02, 0x02, 0x01, 0x00, 0x00, 0x00, 0x00 };
        private static readonly byte[] PhLowCommand8 = { 0x21, 0x02, 0x02, 0x02, 0x00, 0x00, 0x00, 0x00 };

        private const string MatrixIp = "192.168.1.3";
        private const int MatrixHighSlotIndex = 9;
        private const int MatrixLowSlotIndex = 4;

        private readonly A_C_6_15_2_2Simulation _simulation = new A_C_6_15_2_2Simulation();
        ...
        private async Task<bool> SendPhAndMeasureCoreAsync(byte[] cmd8, bool isHigh, CancellationToken token)
        {
            ...
            AddLog($"[{DateTime.Now:HH:mm:ss}] 延时2秒等待波形稳定...");
        ...
        private async Task<bool> EnsureMatrixRoutedAsync(bool isHigh, CancellationToken token)
        {
            ...
            {
                svc.ConnectNodesAsync("I0", "O14", MatrixHighSlotIndex, MatrixIp),
            ...
            _matrixLowRouted = ok;
            _ = token;
            AddLog($"[{DateTime.Now:HH:mm:ss}] 矩阵开关通路(PH{(isHigh ? "高" : "低")}): I0->O14 slot={MatrixHighSlotIndex} & I4->O7 slot={MatrixLowSlotIndex} ip={MatrixIp}, ok={ok}");
            return ok;
        }
        ...
        private async Task DisconnectMatrixAsync(CancellationToken token)
        {
            ...
            try
            {
                if (_matrixHighRouted || _matrixLowRouted)
                {
                    ...
                    {
                        svc.DisconnectNodesAsync("I0", "O14", MatrixHighSlotIndex, MatrixIp),
