...
using MeasureControl.Simulations.A_C_6_13_1_1;
...

{
    ...
    public sealed class A_C_6_13_1_1ViewModel : BindableBase, IDisposable

    {
        ...
        private static readonly byte[] AbCpPressure8 = { 0x07, 0x06, 0x01, 0x01, 0x00, 0x00, 0x00, 0x00 };

        private static readonly byte[] PressureResponsePrefix4 = { 0x07, 0x06, 0x01, 0x01 };

        private static readonly byte[] PressureTelemetryPrefix4 = { 0x07, 0x06, 0x01, 0x02 };
        ...
        private readonly A_C_6_13_1_1Simulation _simulation = new A_C_6_13_1_1Simulation();
        ...
        private static bool IsAbCpPressureResponse(byte[] frameData)

        {
            ...
            return IsPrefix(frameData, PressureResponsePrefix4);

        }



        private static bool TryParsePressureResponse(byte[] frameData, out double pressure)

        {
            ...
            if (!IsPrefix(frameData, PressureResponsePrefix4))

                return false;

            var fbytes = new byte[4] { frameData[4], frameData[5], frameData[6], frameData[7] };
...

        {
...

            {
...

                {
                    ...
                    AddLog($"[{DateTime.Now:HH:mm:ss}] 步骤2：发送测试指令AB_CP_PRESSURE读取压力值");
                    await _simulation.ClearRxFifoAsync(ControllerPressureTestRxChannel);
                    await Task.Delay(20, token);
                    await _simulation.SendBenchCommandOnlyAsync(ControllerPressureTestTxChannel, AbCpPressure8, msg => AddLog(msg), token);

                    var resp = await _simulation.WaitBenchResponse8Async(
                        ControllerPressureTestRxChannel,
                        IsAbCpPressureResponse,
                        timeoutMs: 1500,
                        log: msg => AddLog(msg),
                        token: token);
                    ...
                    ControllerPressureTestRxDataText = "0x" + FormatData(resp);
                    PressureTelemetryRxDataText = "0x" + FormatData(resp);

                    if (!TryParsePressureResponse(resp, out var pressureValue))
                    {
                    ...
                    PressureTelemetryValueText = pressureValue.ToString("0.####", CultureInfo.InvariantCulture);
...

        {
            ...
            AddLog($"[{DateTime.Now:HH:mm:ss}] 档位{gearIndex}：发送AB_CP_PRESSURE");

            await _simulation.SendBenchCommandOnlyAsync(TestTxChannel, AbCpPressure8, msg => AddLog(msg), token);
            ...
            var confirm = await _simulation.WaitBenchResponse8Async(
                ...
                b => b != null && b.SequenceEqual(AbCpPressure8),
            ...
            AddLog($"[{DateTime.Now:HH:mm:ss}] 档位{gearIndex}：等待压力遥测(07 06 01 02)");
...

        {
...

            {
...

                {
                    ...
                    AddLog($"[{DateTime.Now:HH:mm:ss}] 发送AB_CP_PRESSURE：TX={ControllerPressureTestTxChannel}");
