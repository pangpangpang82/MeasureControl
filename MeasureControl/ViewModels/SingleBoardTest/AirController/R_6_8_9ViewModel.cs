    ...
    public class R_6_8_9ViewModel : BindableBase, IDisposable
    {
        ...
        private async Task OnTestTemperatureTelemetryAsync()
        {
            ...
            try
            {
                ...
                if (tempData == null)
                {
                    AddLog($"[{DateTime.Now:HH:mm:ss}] 温度回采值失败：未收到温度采集值(07 01 09 02)");
                ...
                if (rawData != null)
                {
                    ...
                    if (TryParseBase6FromNibbles(rawData, out var rawBase6Decimal))
                        AddLog($"[{DateTime.Now:HH:mm:ss}] 传感器温度原始数据(07 01 09 03) 后四字节(6进制)->10进制：{rawBase6Decimal}，Data={rawHex}");
                    else
                        AddLog($"[{DateTime.Now:HH:mm:ss}] 传感器温度原始数据(07 01 09 03) Data={rawHex}");
                }
            }
            catch (Exception ex)
            {
