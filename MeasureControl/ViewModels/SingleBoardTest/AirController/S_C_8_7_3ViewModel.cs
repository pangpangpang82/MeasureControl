...
namespace MeasureControl.ViewModels.SingleBoardTest.AirController
{
    public sealed class S_C_8_7_3ViewModel : BindableBase, IDisposable
    {
        ...
        public string TestCommandTxDataText { get => "0x" + FormatData(SAftAventsMea038); }
        ...
        private async Task OnTestGearAsync(int gearIndex)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    AddLog($"[{DateTime.Now:HH:mm:ss}] 档位{gearIndex}：AO5输出后等待稳定3s");
                    await Task.Delay(TimeSpan.FromSeconds(3));

                    AddLog($"[{DateTime.Now:HH:mm:ss}] 发送S_AFTAVENTS_MEA03：{FormatData(SAftAventsMea038)}");
        ...
        private async Task OnSendControllerPressureTestAsync()
        {
            ...
            try
            {
                ...
                try
                {
                    AddLog($"[{DateTime.Now:HH:mm:ss}] 发送S_AFTAVENTS_MEA03：TX={ControllerPressureTestTxChannel}");
        ...
        private async Task RunAutoGearAsync(int gearIndex, CancellationToken token, List<string> failures)
        {
            CurrentGearIndex = gearIndex;
            var voltageV = GetGearVoltage(gearIndex);
            AddLog($"[{DateTime.Now:HH:mm:ss}] 自动测试：{CurrentGearName} AO5={voltageV.ToString("0.###", CultureInfo.InvariantCulture)}V");
        ...
        private async Task ResetMtx532OutputToZeroAsync()
        {
            ...
            try
            {
                ...
                await SetAo5Async(0.0, CancellationToken.None);
                VoltageSetValueText = "--";
            }
            catch (Exception ex)
            {
                AddLog($"[{DateTime.Now:HH:mm:ss}] MTX532复位AO5异常：{ex.Message}");
            }
            finally
            {
                _mtxOpLock.Release();
        ...
        private async Task<bool> EnsureMtx532ConnectedAsync(CancellationToken token = default)
        {
            ...
            AddLog($"[{DateTime.Now:HH:mm:ss}] MTX532连接流程开始：参考HC_6_5；硬件AO为1基，目标业务通道={AoChannel}（第5个物理通道）");
        ...
        private void LogTemperatureRawData(byte[] rawData)
        {
            ...
            if (TryParseBase6FromNibbles(rawData, out var rawBase6Decimal))
                AddLog($"[{DateTime.Now:HH:mm:ss}] AFT_AVENTS温度原始数据(15 02 03 03) 后四字节(6进制)->10进制：{rawBase6Decimal}，Data={rawHex}");
            else
                AddLog($"[{DateTime.Now:HH:mm:ss}] AFT_AVENTS温度原始数据(15 02 03 03) Data={rawHex}");
        }
