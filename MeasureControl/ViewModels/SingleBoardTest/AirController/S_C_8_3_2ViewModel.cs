...
using MeasureControl.Simulations.S_C_8_3_2;

namespace MeasureControl.ViewModels.SingleBoardTest.AirController
{
    public sealed class S_C_8_3_2ViewModel : BindableBase, IDisposable
    {
        ...
        private static readonly byte[] ExpectedReceiveResp8 = { 0x13, 0x02, 0x01, 0x02, 0x01, 0x01, 0x01, 0x80 };

        private readonly S_C_8_3_2Simulation _simulation = new S_C_8_3_2Simulation();
        ...
        private async Task OnSendReceiveAsync()
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    AddLog($"[{DateTime.Now:HH:mm:ss}] 发送接收指令：{FormatBytes(SArinc429In01Command8)}");
                    await _simulation.SendBenchCommandOnlyAsync(TestTxChannel, SArinc429In01Command8, msg => AddLog(msg), token);

                    var resp8 = await _simulation.WaitBenchResponse8Async(
                        TestRxChannel,
                        b => b != null && b.Length == 8 && b[0] == SArinc429In01OkPrefix4[0] && b[1] == SArinc429In01OkPrefix4[1] && b[2] == SArinc429In01OkPrefix4[2] && b[3] == SArinc429In01OkPrefix4[3],
                        timeoutMs: 1200,
                        log: msg => AddLog(msg),
                        token: token);
