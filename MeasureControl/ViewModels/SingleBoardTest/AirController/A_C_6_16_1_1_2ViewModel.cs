...
using MeasureControl.Simulations.A_C_6_16_1_1_2;

namespace MeasureControl.ViewModels.SingleBoardTest.AirController
{
    public sealed class A_C_6_16_1_1_2ViewModel : BindableBase, IDisposable
    {
        ...
        public A_C_6_16_1_1_2ViewModel()
        {
            ManualTestCommand = new DelegateCommand(OnManualTest);
            AutoTestCommand = new DelegateCommand(OnAutoTest);
            ClearLogCommand = new DelegateCommand(() => Logs.Clear());

            SendEnterAtpCommand = new DelegateCommand(async () => await SendAndWaitOkAsync(EnterAtpCommand8, null, "进入ATP"));
            SendExitAtpCommand = new DelegateCommand(async () => await SendAndWaitOkAsync(ExitAtpCommand8, null, "退出ATP"));

            SendSupImpedanceCommand = new DelegateCommand(async () => await SendAndWaitEchoAsync(SupImpedanceCommand8, "AB_MOTORCKPTTAV_SUPIMPEDANCE"));
        ...
        private async Task RunAutoTestAsync()
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    if (!await AutoStepAsync(EnterAtpCommand8, null, "进入ATP", token))
                        failures.Add("进入ATP失败");

                    if (!await AutoStepAsync(SupImpedanceCommand8, SupImpedanceCommand8, "AB_MOTORCKPTTAV_SUPIMPEDANCE", token))
                        failures.Add("AB_MOTORCKPTTAV_SUPIMPEDANCE回读失败");
                    else if (!await MeasureAndQualifyAsync(token))
                        failures.Add("占空比/频率判据不合格");

                    if (!await AutoStepAsync(ExitAtpCommand8, null, "退出ATP", token))
        ...
        {
            // Cancel CTS and force-close streams immediately — don't wait for _autoTestLock to avoid deadlock
            ...
            try
            {
                ...
                await _autoTestLock.WaitAsync(cts.Token);
            }
            catch (OperationCanceledException)
            {
