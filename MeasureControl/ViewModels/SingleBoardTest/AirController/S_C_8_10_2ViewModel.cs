...
{
    public sealed class S_C_8_10_2ViewModel : BindableBase, IDisposable
    {
        ...
        private static readonly (string In, string Out, int Slot) MatrixPointJ99J100 = ("I1", "O9", MatrixSlotSig);
        ...
        private async Task<bool> EnsureMatrixRoutedAsync(CancellationToken token)
        {
            ...
            AddLog($"矩阵路由: slot{MatrixSlotDmm} {MatrixDmmH.In}-{MatrixDmmH.Out} + slot{MatrixSlotSig} {MatrixPointJ99J100.In}-{MatrixPointJ99J100.Out}");
            ...
            _matrixRoutedSig = await matrix.ConnectNodesAsync(MatrixPointJ99J100.In, MatrixPointJ99J100.Out, MatrixPointJ99J100.Slot, MatrixIpAddress, MatrixTcpBasePort).ConfigureAwait(false);
        ...
        private async Task UnrouteMatrixAsync(CancellationToken token)
        {
            ...
            {
                if (_matrixRoutedSig)
                    _ = await matrix.DisconnectNodesAsync(MatrixPointJ99J100.In, MatrixPointJ99J100.Out, MatrixPointJ99J100.Slot, MatrixIpAddress, MatrixTcpBasePort).ConfigureAwait(false);
            }
            catch { }
