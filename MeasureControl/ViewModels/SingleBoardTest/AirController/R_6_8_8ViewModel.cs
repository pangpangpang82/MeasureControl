...
namespace MeasureControl.ViewModels.SingleBoardTest.AirController
{
    public class R_6_8_8ViewModel : BindableBase, IDisposable
    {
        ...
        private static readonly byte[] TemperatureTelemetryCommand = { 0x07, 0x01, 0x08, 0x02, 0x00, 0x00, 0x00, 0x00 };
        ...
        private async Task OnTestTemperatureTelemetryAsync()
        {
            ...
            try
            {
                ...
                if (temperature == null)
                {
                    ...
                    AddLog($"[{DateTime.Now:HH:mm:ss}] 温度回采值失败：{elapsed}ms内未收到温度采集值(07 01 08 02)");
                ...
                if (rawData != null)
                {
                    ...
                    if (TryParseBase6FromNibbles(rawData, out var rawBase6Decimal))
                        AddLog($"[{DateTime.Now:HH:mm:ss}] 传感器温度原始数据(07 01 08 03) 后四字节(6进制)->10进制：{rawBase6Decimal}，Data={rawHex}");
                    else
                        AddLog($"[{DateTime.Now:HH:mm:ss}] 传感器温度原始数据(07 01 08 03) Data={rawHex}");
                }
            }
            catch (Exception ex)
            {
