...
namespace MeasureControl.ViewModels.SingleBoardTest.AirController
{
    public class R_6_8_8ViewModel : BindableBase, IDisposable
    {
        ...
        private async Task OnTestControllerTemperatureAsync()
        {
            ...
            try
            {
                AddLog($"[{DateTime.Now:HH:mm:ss}] 测试：PTS温度，TX={ControllerTemperatureTestTxChannel}, Label=0x{DefaultLabel:X2}");
        ...
        private async Task OnTestTemperatureTelemetryAsync()
        {
            ...
            try
            {
                ...
                var temp8 = _lastTemperatureTelemetryFrame;
                var tempC = _lastTelemetryTemperatureC;
                ...
                if (tempC.HasValue)
                {
                    TemperatureTelemetryValueText = $"{tempC.Value:F2}°C";
                    AddLog($"[{DateTime.Now:HH:mm:ss}] 温度回采值={tempC.Value:F2}°C");
                }
                else
                {
                    AddLog($"[{DateTime.Now:HH:mm:ss}] 缓存中无温度遥测帧");
                }
            }
            catch (Exception ex)
            {
                ...
                while (!token.IsCancellationRequested)
                {
                    try
                    {
                        ...
                        if (tempData != null)
                        {
                            ...
                            var tempValue = ParseTemperatureFromBytes(tempData);
                            _lastTelemetryTemperatureC = tempValue;
