...
{
    public class A28vOc400mACh1TestViewModel : BindableBase
    {
        ...
        private static readonly byte[] A_28VDSOH_28VTEST = { 0x09, 0x04, 0x01, 0x01, 0x00, 0x00, 0x00, 0x00 };
        private static readonly byte[] A_28VDSOH_28VTEST_ACK = { 0x09, 0x04, 0x01, 0x02, 0xAA, 0xAA, 0xAA, 0xAA };
        private static readonly byte[] A_28VDSOH_28VTEST2 = { 0x09, 0x04, 0x01, 0x03, 0x00, 0x00, 0x00, 0x00 };
        private static readonly byte[] A_28VDSOH_OCTEST = { 0x09, 0x04, 0x01, 0x06, 0x00, 0x00, 0x00, 0x00 };
        private static readonly byte[] A_28VDSOH_OCTEST_ACK = { 0x09, 0x04, 0x01, 0x07, 0xAA, 0xAA, 0xAA, 0xAA };
        private static readonly byte[] A_28VDSOH_OCTEST2 = { 0x09, 0x04, 0x01, 0x08, 0x00, 0x00, 0x00, 0x00 };
        ...
        public A28vOc400mACh1TestViewModel()
        {
            AutoTestCommand = new DelegateCommand(OnAutoTest);
            ClearLogCommand = new DelegateCommand(() => Logs.Clear());
        }

        public string PageTitle => "6.15.3.1 28V/OC型400mA离散输出通道1输出测试";
        ...
        {
            // (2) 429发送测试指令A_28VDSOH_28VTEST 0x09 04 01 01 00 00 00 00
            ...
            // 用万用表直流电压挡位和矩阵开关测量负载两端实际电压值，3022的矩阵开关I0 c35（slot2） 和r4  c0(slot4)
            ...
            // (4) 429发送测试指令A_28VDSOH_28VTEST2 0x09 04 01 03 00 00 00 00；
            ...
            var loopResp = await SendAndReadWithRetryAsync(
                A_28VDSOH_28VTEST2,
                b => b != null && b.Length == 8 && b[0] == 0x09 && b[1] == 0x04 && b[2] == 0x01 && b[3] == 0x04,
                2000,
                token);
        ...
        {
            // (6) 429发送测试指令A_28VDSOH_OCTEST 0x09 04 01 06 00 00 00 00
            ...
            // (7) 429发送测试指令A_28VDSOH_OCTEST2 0x09 04 01 08 00 00 00 00；
            AddLog($"[{DateTime.Now:HH:mm:ss}] (7) 发送A_28VDSOH_OCTEST2：{FormatBytesHex(A_28VDSOH_OCTEST2)}");
            var loopResp = await SendAndReadWithRetryAsync(
                A_28VDSOH_OCTEST2,
                b => b != null && b.Length == 8 && b[0] == 0x09 && b[1] == 0x04 && b[2] == 0x01 && b[3] == 0x09,
                2000,
                token);
