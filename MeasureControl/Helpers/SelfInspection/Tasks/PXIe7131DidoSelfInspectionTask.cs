...
namespace MeasureControl.Helpers.SelfInspection
{
    internal sealed class PXIe7131DidoSelfInspectionTask : ISelfInspectionTask
    {
        ...
        private static async Task RecordRangeOnceAsync(IDeviceDriver driver, SelfInspectionContext context, CancellationToken cancellationToken, int start, int count)
        {
            ...
            context.Log($"DI{start + 1}~DI{start + count}：{FormatBitsRange(di, "DI", start, count)}");
            context.Log($"DO{start + 1}~DO{start + count}：{FormatBitsRange(doVals, "DO", start, count)}");
        }
            ...
            int expectedBit)
        {
            ...
            context.Log($"DI{start + 1}~DI{start + count} 判定期望={expectedBit} => {(ok ? "OK" : "NG")}");
            return ok;
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                foreach (var doIndex in doIndices)
                {
                    ...
                    {
                        allOk = false;
                        context.Log($"DO{doIndex + 1}: 矩阵选择链路连接失败 {MatrixSelectInputNodeId}->{curOutput} slot={MatrixSelectSlotIndex}");
                    ...
                    context.Log($"DO{doIndex + 1} 电压: {measured:F4}V, 期望={expectedVoltage:F3}V, 范围=[{low:F3},{high:F3}] => {(ok ? "OK" : "NG")}");
                }
        ...
        public async Task RunAsync(DeviceBase device, SelfInspectionContext context, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                {
                    context.Log("自检报表路径为空，跳过写入报表（请检查是否由SelfInspectionRunner生成报表）。");
                }
