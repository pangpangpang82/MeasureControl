...
{
    internal sealed class PXI4087CSelfInspectionTask : ISelfInspectionTask
    {
        ...
        private static bool Is4087C(DeviceBase device)
        {
            var model = (device?.Model ?? string.Empty).ToUpperInvariant();
            return model.Contains("4087C");
        }
        ...
        {
            return device != null && Is4087C(device);
        }
        ...
        public async Task RunAsync(DeviceBase device, SelfInspectionContext context, CancellationToken cancellationToken)
        {
            ...
            if (!Is4087C(device))
            {
                context.Log($"跳过：非4087C板卡 {device.Name} Model={device.Model}");
                return;
            }
            ...
            try
            {
                ...
                context.Log($"准备打开板卡 4087C (Slot={slotIndex})");
