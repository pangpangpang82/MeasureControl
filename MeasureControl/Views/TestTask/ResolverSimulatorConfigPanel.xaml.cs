...
{
    public partial class ResolverSimulatorConfigPanel : UserControl
    {
        private ResolverSimulatorConfigPanelViewModel Vm => DataContext as ResolverSimulatorConfigPanelViewModel;
        private string _originalCardName;

        public ResolverSimulatorConfigPanel()
        {
            InitializeComponent();
            this.DataContextChanged += ResolverSimulatorConfigPanel_DataContextChanged;
        ...
        private void ResolverSimulatorConfigPanel_DataContextChanged(object sender, DependencyPropertyChangedEventArgs e)
        {
            if (e.OldValue is ResolverSimulatorConfigPanelViewModel oldVm)
            {
                oldVm.WaveformUpdated -= OnWaveformUpdated;
                oldVm.WaveformUpdatedVaVb -= OnWaveformUpdatedVaVb;
                System.Diagnostics.Debug.WriteLine("[UI波形] Resolver: 取消订阅波形更新事件");
            }
            if (e.NewValue is ResolverSimulatorConfigPanelViewModel newVm)
            {
                newVm.WaveformUpdated += OnWaveformUpdated;
                newVm.WaveformUpdatedVaVb += OnWaveformUpdatedVaVb;
                System.Diagnostics.Debug.WriteLine("[UI波形] Resolver: 已订阅波形更新事件");
            }
        }
        ...
        private void OnWaveformUpdated(double[] samples)
        {
            ...
            Dispatcher.Invoke(() =>
            {
                ...
                if (canvas != null)
                {
                    System.Diagnostics.Debug.WriteLine($"[UI波形] Resolver: 找到输出Canvas，正在绘制波形，样本数量: {samples?.Length ?? 0}");
                ...
                {
                    System.Diagnostics.Debug.WriteLine("[UI波形] Resolver: 未找到输出Canvas，无法绘制波形");
                }
            });
        }
        ...
        private void OnWaveformUpdatedVaVb(double[] vaSamples, double[] vbSamples)
        {
            Dispatcher.Invoke(() =>
            {
                ...
                {
                    System.Diagnostics.Debug.WriteLine("[UI波形] Resolver: 未找到激励Canvas，无法绘制VaVb波形");
                }
            });
        }
        ...
        {
            var named = FindVisualChild<Canvas>(this, "ResolverWaveformCanvas_Excitation");
            if (named != null && named.IsVisible) return named;
            return FindVisibleWaveformCanvas(this);
        }

        private Canvas FindOutputCanvas()
        {
            var named = FindVisualChild<Canvas>(this, "ResolverWaveformCanvas_Output");
            if (named != null && named.IsVisible) return named;
            // fallback to any visible waveform canvas
            return FindVisibleWaveformCanvas(this);
        }
        ...
        private Canvas FindVisibleWaveformCanvas(DependencyObject parent)
        {
            ...
            // 首先尝试通过Name查找
            var namedCanvas = FindVisualChild<Canvas>(parent, "ResolverWaveformCanvas");
        ...
        private void NumericTextBox_PreviewTextInput(object sender, System.Windows.Input.TextCompositionEventArgs e)
        {
            ...
            if (e.Text == ".")
            {
                ...
                if (insertPos == 0)
                {
                    // 在开头插入小数点，允许（会变成0.xxxx）
                    return;
                }
                else if (insertPos > 0)
                {
                    // 检查前面是否有非数字字符（除了负号和小数点）
                    string beforeInsert = currentText.Substring(0, insertPos);
                    // 允许在数字后面或负号后面插入小数点
                    if (System.Text.RegularExpressions.Regex.IsMatch(beforeInsert, @"^-?\d*$"))
                    {
                        return; // 允许输入
                    }
                    else
